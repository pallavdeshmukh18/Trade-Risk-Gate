...
export async function updateMarket(symbol, ohlcv, options = {}) {
    ...
    await safeRedisSet(`${ohlcvKey}:${symbol}`, JSON.stringify(ohlcv));

    if (updateVolatility) {
        const rets = closes.length > 1 ? calcReturns(closes) : [];
        const vol = rets.length > 0 ? calcVol(rets) : 0;
        await safeRedisSet(`volatility:${symbol}`, vol);
    }

    if (updatePrice && price != null) {
        await safeRedisSet(`live_price:${symbol}`, price);
    }

    return {
        price,
        volatility: updateVolatility ? await safeRedisGet(`volatility:${symbol}`) : null,
    };
}
